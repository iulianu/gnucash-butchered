/*
 * dialog-search.cpp -- Search Dialog
...
gnc_search_dialog_add_criterion (GNCSearchWindow *sw)
{
    ...
    _new = gnc_search_core_type_new_type_name
    ...
    if (_new)
    {
        ...
        gnc_search_core_type_grab_focus (_new);
        gnc_search_core_type_editable_enters (_new);
    }
}
...
type_label_to_new_button(const gchar* type_label)
{
    ...
    {
        PWARN("No translatable new-button label found for search type \"%s\", please add one into dialog-search.cpp!", type_label);
        return _("New item");
    }
}
