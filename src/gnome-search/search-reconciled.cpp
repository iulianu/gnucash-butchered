...
{
    o->how = static_cast<int>(QOF_COMPARE_EQUAL);
    o->value = CLEARED_NO;
}
...
toggle_changed (GtkToggleButton *button, GNCSearchReconciled *fe)
{
    ...
    cleared_match_t value =
        reinterpret_cast<int>( g_object_get_data (G_OBJECT (button), "button-value") );
...
make_menu (GNCSearchCoreType *fe)
{
    ...
    gnc_combo_box_search_changed(combo, reinterpret_cast<guint*>(&fi->how));
...
gncs_get_widget (GNCSearchCoreType *fe)
{
    ...
    toggle = make_toggle (fi, _("Not Cleared"), static_cast<QofCharMatch>(CLEARED_NO));
    gtk_box_pack_start (GTK_BOX (box), toggle, FALSE, FALSE, 3);

    toggle = make_toggle (fi, _("Cleared"), static_cast<QofCharMatch>(CLEARED_CLEARED));
    gtk_box_pack_start (GTK_BOX (box), toggle, FALSE, FALSE, 3);

    toggle = make_toggle (fi, _("Reconciled"), static_cast<QofCharMatch>(CLEARED_RECONCILED));
    gtk_box_pack_start (GTK_BOX (box), toggle, FALSE, FALSE, 3);

    toggle = make_toggle (fi, _("Frozen"), static_cast<QofCharMatch>(CLEARED_FROZEN));
    gtk_box_pack_start (GTK_BOX (box), toggle, FALSE, FALSE, 3);

    toggle = make_toggle (fi, _("Voided"), static_cast<QofCharMatch>(CLEARED_VOIDED));
    gtk_box_pack_start (GTK_BOX (box), toggle, FALSE, FALSE, 3);
