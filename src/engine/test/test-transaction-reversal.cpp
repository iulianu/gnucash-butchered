...
run_test (void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        _new = xaccSplitGetAmount(xaccTransGetSplit(new_trans, i));
        if (gnc_numeric_eq(old, gnc_numeric_neg(_new)))
        {
        ...
        _new = xaccSplitGetValue(xaccTransGetSplit(new_trans, i));
        if (gnc_numeric_eq(old, gnc_numeric_neg(_new)))
        {
