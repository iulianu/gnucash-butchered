/********************************************************************
 * utest-Account.cpp: GLib g_test test suite for Account.cpp.	    *
...
insert_split (Account *parent, Transaction *txn, SplitParms *p)
{
    ...
    LotList_t lotlist = xaccAccountGetLotList (acct);
    GNCLot* lot = NULL;
    g_assert (acct != NULL);
    ...
    if (p->lotnum > lotlist.size())
        lot = gnc_lot_new (book);
    else
    {
        LotList_t::iterator iter = lotlist.begin();
        std::advance(iter, p->lotnum - 1);
        lot = *iter;
    }
...
test_gnc_account_create_and_destroy (void)
{
    ...
    g_assert (xaccAccountGetLotList (acc).empty());
...
test_xaccCloneAccount (Fixture *fixture, gconstpointer pData)
{
    ...
    g_assert (kvp_frame_compare (clone->kvp_data,
                                 fixture->acct->kvp_data) == 0);
...
test_xaccAccountCommitEdit (Fixture *fixture, gconstpointer pData)
{
    ...
    sig1 = test_signal_new (parent, QOF_EVENT_MODIFY, NULL);
    sig2 = test_signal_new (parent, QOF_EVENT_DESTROY, NULL);
...
//    gchar *log_domain = "gnc.engine";
    ...
    guint logger;
    sig1 = test_signal_new (fixture->acct, QOF_EVENT_MODIFY, NULL);
    sig2 = test_signal_new (fixture->acct, GNC_EVENT_ITEM_ADDED, split1);
    ...
    sig3 = test_signal_new (fixture->acct, GNC_EVENT_ITEM_ADDED, split3);
...
test_xaccAccountInsertRemoveLot (Fixture *fixture, gconstpointer pData)
{
    ...
    TestSignal sig1 = test_signal_new (QOF_INSTANCE (lot),
                                       QOF_EVENT_ADD, NULL);
    TestSignal sig2 = test_signal_new (fixture->acct,
                                       QOF_EVENT_MODIFY, NULL);
    TestSignal sig3 = test_signal_new (QOF_INSTANCE (lot),
                                       QOF_EVENT_REMOVE, NULL);
    TestSignal sig4 = test_signal_new (parent, QOF_EVENT_MODIFY, NULL);
...
test_gnc_account_append_remove_child (Fixture *fixture, gconstpointer pData)
{
    ...
    test_signal_free (sig2);
    test_signal_free (sig3);
    sig1 = test_signal_new (account, QOF_EVENT_REMOVE, NULL);
    sig2 = test_signal_new (fixture->acct, QOF_EVENT_MODIFY, NULL);
...
test_xaccAccountType_Stuff (void)
{
    ...
    for (type = ACCT_TYPE_NONE; type < ACCT_TYPE_LAST; type = static_cast<int>(type) + 1)
    {
...
test_xaccAccountType_Compatibility (void)
{
    ...
    for (type = ACCT_TYPE_BANK; type < NUM_ACCOUNT_TYPES; type = static_cast<int>(type) + 1)
    {
        ...
        for (child = ACCT_TYPE_NONE; child < ACCT_TYPE_LAST; child = static_cast<int>(child) + 1)
            if (1 << child & compat)
