/********************************************************************
 * io-gncxml-v1.cpp -- read XML-format gnucash data file            *
        ...
        sixtp_child_result *child_result)
{
    ...
    {
        if (cpi->comm_namespace) return(FALSE);
        cpi->comm_namespace = (gchar *) child_result->data;
        child_result->should_cleanup = FALSE;
    }
        ...
        gpointer *result, const gchar *tag)
{
    ...
    if (cpi->comm_namespace && cpi->id)
    {
        ...
        com = gnc_commodity_table_lookup(table, cpi->comm_namespace, cpi->id);
