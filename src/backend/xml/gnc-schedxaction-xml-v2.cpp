/********************************************************************
 * gnc-schedxactions-xml-v2.cpp -- xml routines for transactions    *
...
gnc_schedXaction_dom_tree_create(SchedXaction *sx)
{
    ...
    /* FIXME: this should be the same as the def in io-gncxml-v2.cpp */
    ...
    else
    {
        ...
        if ( xaccAccountGetCommodity( acc ) == NULL )
        {
            ...
            /* FIXME: This should first look in the table of the
               ...
               applies for
               SchedXaction.cpp:xaccSchedXactionInit... */
