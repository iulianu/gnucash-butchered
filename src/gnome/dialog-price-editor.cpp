/********************************************************************\
 * dialog-price-editor.cpp -- price editor dialog                   *
...
price_to_gui (PriceEditDialog *pedit_dialog)
{
    ...
    const gchar *comm_namespace, *fullname;
    const char *source;
    const char *type;
    gnc_numeric value;
    Timespec date;
...
gui_to_price (PriceEditDialog *pedit_dialog)
{
    ...
    comm_namespace = gnc_ui_namespace_picker_ns (pedit_dialog->namespace_cbwe);
    fullname = gtk_entry_get_text( GTK_ENTRY( gtk_bin_get_child( GTK_BIN( GTK_COMBO_BOX(pedit_dialog->commodity_cbwe)))));

    commodity = gnc_commodity_table_find_full(gnc_get_current_commodities(), comm_namespace, fullname);
    if (!commodity)
...
pedit_commodity_ns_changed_cb (GtkComboBox *cbwe, gpointer data)
{
    ...
    comm_namespace = gnc_ui_namespace_picker_ns (pedit_dialog->namespace_cbwe);
    gnc_ui_update_commodity_picker (pedit_dialog->commodity_cbwe, comm_namespace, NULL);
