...
static GList * copy_and_terms (const GList *and_terms)
{
    ...
    {
        l_and = g_list_prepend(l_and, copy_query_term (cur_and->data));
    }

    return g_list_reverse(l_and);
}
...
copy_or_terms(const GList * or_terms)
{
    ...
    {
        l_or = g_list_prepend(l_or, copy_and_terms(cur_or->data));
    }

    return g_list_reverse(l_or);
}
...
void qof_query_purge_terms (QofQuery *q, QofQueryParamList *param_list)
{
    ...
    for (l_or = q->terms; l_or; l_or = l_or->next)
    {
        for (l_and = l_or->data; l_and; l_and = l_and->next)
        {
            qt = l_and->data;
            if (!param_list_cmp (qt->param_list, param_list))
            {
                if (g_list_length (l_or->data) == 1)
                {
                    q->terms = g_list_remove_link (q->terms, l_or);
                    g_list_free_1 (l_or);
                    l_or = q->terms;
                    break;
                }
                ...
                {
                    l_or->data = g_list_remove_link (l_or->data, l_and);
                    g_list_free_1 (l_and);
                    l_and = l_or->data;
                    if (!l_and) break;
                }
