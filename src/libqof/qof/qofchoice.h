...
*/
bool qof_object_is_choice(QofIdTypeConst type);

/** \brief Set an object as using QOF_TYPE_CHOICE. */
bool  qof_choice_create(char* type);
...
@return FALSE if object is not a choice object or on error
	otherwise TRUE.
*/
bool qof_choice_add_class(const char* choice, char* add, char* param_name);
...
*/
bool qof_choice_check(const char* choice_obj,
