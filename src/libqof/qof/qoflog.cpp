...
{
    return static_cast<QofLogLevel>(static_cast<int>(gll));
}
...
log4glib_handler(const gchar     *log_domain,
                 ...
                 gpointer        user_data)
{
    if (G_LIKELY(!qof_log_check(log_domain, gLogLevelToQofLogLevel(log_level))))
        return;

    {
        ...
        const char *format_24hour =
            "%T"
            ;
        gchar *level_str = qof_log_level_to_string(gLogLevelToQofLogLevel(log_level));
        now = gnc_time (NULL);
