...
iter_to_string (GncTreeModelPrice *model, GtkTreeIter *iter)
{
    ...
    if (iter)
    {
        if(GPOINTER_TO_INT(iter->user_data) == GPOINTER_TO_INT(ITER_IS_NAMESPACE))
        {
            comm_namespace = (gnc_commodity_namespace *) iter->user_data2;
            snprintf(string, ITER_STRING_LEN,
                     "[stamp:%x data:%d (NAMESPACE), %p (%s), %d]",
                     iter->stamp, GPOINTER_TO_INT(iter->user_data),
                     iter->user_data2, gnc_commodity_namespace_get_name (comm_namespace),
                     GPOINTER_TO_INT(iter->user_data3));
        }
        else if(GPOINTER_TO_INT(iter->user_data) == GPOINTER_TO_INT(ITER_IS_COMMODITY))
        {
        ...
        else if(GPOINTER_TO_INT(iter->user_data) == GPOINTER_TO_INT(ITER_IS_PRICE))
        {
                               ...
                               GtkTreePath *path)
{
    ...
    comm_namespace = g_list_nth_data (ns_list, i);
    if (!comm_namespace)
    {
    ...
    if (depth == 1)
    {
        ...
        iter->user_data2 = comm_namespace;
    ...
    cm_list = gnc_commodity_namespace_get_commodity_list(comm_namespace);
                               ...
                               GtkTreeIter *iter)
{
    ...
    if (iter->user_data == ITER_IS_COMMODITY)
    {
        ...
        commodity = (gnc_commodity*)iter->user_data2;
        comm_namespace = gnc_commodity_get_namespace_ds(commodity);
        path = gtk_tree_path_new ();
        gtk_tree_path_append_index (path, g_list_index (ns_list, comm_namespace));
        gtk_tree_path_append_index (path, GPOINTER_TO_INT(iter->user_data3));
        debug_path(LEAVE, path);
        return path;
    }
                                ...
                                GValue *value)
{
    ...
    if (iter->user_data == ITER_IS_NAMESPACE)
    {
        ...
        case GNC_TREE_MODEL_PRICE_COL_COMMODITY:
            ...
            g_value_set_string (value, gnc_commodity_namespace_get_name (comm_namespace));
            break;
        case GNC_TREE_MODEL_PRICE_COL_VISIBILITY:
                                ...
                                GtkTreeIter *iter)
{
    ...
    {
        comm_namespace = gnc_commodity_get_namespace_ds((gnc_commodity *)iter->user_data2);
                                    ...
                                    GtkTreeIter *parent)
{
    ...
    {
        comm_namespace = (gnc_commodity_namespace *)parent->user_data2;
        list = gnc_commodity_namespace_get_commodity_list(comm_namespace);
        if (list == NULL)
        {
                                  ...
                                  GtkTreeIter *child)
{
    ...
    if (child->user_data == ITER_IS_COMMODITY)
    {
        ct = qof_book_get_data (priv->book, GNC_COMMODITY_TABLE);
        list = gnc_commodity_table_get_namespaces_list(ct);
        comm_namespace = gnc_commodity_get_namespace_ds((gnc_commodity*)child->user_data2);

        iter->stamp      = model->stamp;
        iter->user_data  = ITER_IS_NAMESPACE;
        iter->user_data2 = comm_namespace;
        iter->user_data3 = GINT_TO_POINTER(g_list_index(list, comm_namespace));
        LEAVE("ns iter %p (%s)", iter, iter_to_string(model, iter));
        return TRUE;
    }
        ...
        GtkTreeIter *iter)
{
    ...
    ENTER("model %p, namespace %p, iter %p", model, comm_namespace, iter);
    g_return_val_if_fail (GNC_IS_TREE_MODEL_PRICE (model), FALSE);
    g_return_val_if_fail ((comm_namespace != NULL), FALSE);
    g_return_val_if_fail ((iter != NULL), FALSE);
    ...
    n = g_list_index(list, comm_namespace);
    if (n == -1)
        return FALSE;
                                    ...
                                    gpointer event_data)
{
    ...
    else if (GNC_IS_COMMODITY_NAMESPACE(entity))
    {
        ...
        if (event_type != QOF_EVENT_DESTROY)
        {
            if (!gnc_tree_model_price_get_iter_from_namespace (model, comm_namespace, &iter))
            {
                LEAVE("no iter");
                return;
            }
