                               ...
                               dialog_commodity_mode mode)
{
    ...
    gnc_ui_update_namespace_picker(retval->namespace_combo,
                                   ...
                                   mode);
    comm_namespace = gnc_ui_namespace_picker_ns(retval->namespace_combo);
    gnc_ui_update_commodity_picker(retval->commodity_combo, comm_namespace,
                               ...
                               gpointer user_data)
{
    ...
    gchar * comm_namespace = gnc_ui_namespace_picker_ns (w->namespace_combo);

    const gnc_commodity * new_commodity =
        gnc_ui_new_commodity_modal_full(comm_namespace,
                                        w->dialog,
                                        w->default_cusip,
                                        w->default_fullname,
                                        w->default_mnemonic,
        ...
        gpointer user_data)
{
    ...
    ENTER("cbwe=%p, user_data=%p", cbwe, user_data);
    comm_namespace = gnc_ui_namespace_picker_ns (w->namespace_combo);
    DEBUG("namespace=%s", comm_namespace);
    gnc_ui_update_commodity_picker(w->commodity_combo, comm_namespace, NULL);
    g_free(comm_namespace);
    LEAVE(" ");
}
                                ...
                                const gchar * init_string)
{
    ...
    g_return_if_fail(comm_namespace);
...
gnc_ui_select_commodity_response_cb (GtkDialog * dialog, gint response, gpointer data)
{
    ...
    case GNC_RESPONSE_NEW:
        ...
        commodity = gnc_ui_new_commodity_modal_full (comm_namespace,
                    w->dialog,
                    w->default_cusip,
                    w->default_fullname,
                    w->default_mnemonic,
...
gnc_ui_namespace_picker_ns (GtkWidget *cbwe)
{
    ...
    comm_namespace = gtk_entry_get_text( GTK_ENTRY( gtk_bin_get_child( GTK_BIN( GTK_COMBO_BOX(cbwe)))));
    ...
    if (g_strcmp0 (comm_namespace, GNC_COMMODITY_NS_ISO) == 0)
    {
        ...
        return g_strdup(comm_namespace);
}
...
gnc_ui_commodity_changed_cb(GtkWidget * dummy, gpointer user_data)
{
    ...
    if (!w->is_currency)
    {
        ...
        DEBUG("namespace=%s, name=%s, mnemonic=%s", comm_namespace, fullname, mnemonic);
        ok = (fullname    && comm_namespace    && mnemonic &&
              fullname[0] && comm_namespace[0] && mnemonic[0]);
        g_free(comm_namespace);
    }
    else
    {
                              ...
                              int fraction)
{
    ...
    if (commodity)
    {
        comm_namespace = gnc_commodity_get_namespace (commodity);
    ...
    win = gnc_ui_build_commodity_dialog(comm_namespace, parent, fullname,
 ...
 ********************************************************/
gnc_commodity *
gnc_ui_new_commodity_modal_full(const char * comm_namespace,
                                GtkWidget * parent,
                                const char * cusip,
                                const char * fullname,
                                const char * mnemonic,
                                int fraction)
{
    ...
    ENTER(" ");
    result = gnc_ui_common_commodity_modal(NULL, parent, comm_namespace, cusip,
...
gnc_ui_commodity_dialog_to_object(CommodityWindow * w)
{
            ...
            mnemonic && mnemonic[0])
    {
        ...
        if (!w->edit_commodity)
        {
            c = gnc_commodity_new(book, fullname, comm_namespace, mnemonic, code, fraction);
            w->edit_commodity = c;
            gnc_commodity_begin_edit(c);
        }
        else
        {
            ...
            gnc_commodity_set_namespace (c, comm_namespace);
        ...
        for (type = SOURCE_SINGLE; type < SOURCE_MAX; type = static_cast<int>(type) + 1)
        {
